...
TfLiteStatus HardSwishPrepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (input->type == kTfLiteInt8 || input->type == kTfLiteInt16) {
    ...
    const float hires_input_scale =
        (1.0f / 128.0f) * input_scale;  // Used for int8 only
    const float reluish_scale = 3.0f / 32768.0f;
    const float output_scale = output->params.scale;

    double output_multiplier;
    if (input->type == kTfLiteInt16) {
      output_multiplier = static_cast<double>(input_scale / output_scale);
    } else {
      output_multiplier = static_cast<double>(hires_input_scale / output_scale);
    }
    ...
    double reluish_multiplier;
    if (input->type == kTfLiteInt16) {
      reluish_multiplier = static_cast<double>(input_scale / reluish_scale);
    } else {
      reluish_multiplier =
